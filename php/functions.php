...
/**
 * Connects to Emerald daemon and retrieves information, then writes to cache
 *
...
function getData($from_cache = false)
{
    ...
    include_once './php/easyemerald.php';
    ...
    // Get info
    $data = $emerald->getinfo();

    // Handle errors if they happened
    if (!$data) {
        $return_data['error'] = $emerald->error;
        $return_data['status'] = $emerald->status;
    ...
    if (isset($config['display_ip']) && $config['display_ip'] === true) {
        // Use emeraldd IP
        if ($config['use_emeraldd_ip'] === true) {
            $net_info = $emerald->getnetworkinfo();
    ...
    if ($config['display_peer_info'] === true) {
        $data['peers'] = parsePeers($emerald->getpeerinfo());
    }
    ...
    // Emerald Daemon uptime
    if (($config['display_emeraldd_uptime'] === true) || (strcmp(PHP_OS, "Linux") == 0)) {
        $data['emeraldd_uptime'] = getProcessUptime($config['emeraldd_process_name']);
    }
    ...
    if ($config['display_max_height'] === true) {
        ...
        curl_setopt($bitnodes_ch, CURLOPT_USERAGENT, 'Emerald Node Status Page');
...
function getGeolocation($ip_address, $response_key)
{
    ...
    curl_setopt($ch, CURLOPT_USERAGENT, 'Emerald Node Status Page');
